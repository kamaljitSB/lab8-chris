...
def check():
    ...
    try:
        res = requests.get("http://35.192.222.219/receiver/health")
    ...
    try:
        res = requests.get("http://35.192.222.219/storage/health")
        if res.status_code == 200:
            statuses["storage"] = "Up"
        else:
    ...
    try:
        res = requests.get("http://35.192.222.219/processing/health")
        if res.status_code == 200:
